...
ALLOWED_STATES = {"backlog", "ready", "in-progress", "review", "blocked", "done", "dropped"}
...
def validate_state(state: str) -> str:
    if state not in ALLOWED_STATES:
        raise SystemExit(f"État illégal : '{state}'. Autorisés : {', '.join(sorted(ALLOWED_STATES))}.")
    return state
...
def transition(tracker, issue_id: str, state: str, context=None) -> None:
    ...
    if (getattr(tracker, "bounded_transition_proofs", False)
            and normalized in {"review", "done"} and context is None):
        raise SystemExit(
...
def update_issue_body(tracker, issue_id: str, expected_body: str, updated_body: str) -> bool:
    """Rewrite one issue body through the provider-agnostic bounded body port."""
    binding = issue_binding(tracker, issue_id)
    kwargs = {"project": binding} if binding is not None else {}
    return tracker.update_body(Issue(id=issue_id, title=""), expected_body, updated_body, **kwargs)


def update_adr_body(tracker, adr_id: str, expected_body: str, updated_body: str) -> bool:
    """Rewrite one ADR body through the provider-agnostic bounded body port."""
    binding = adr_binding(tracker, adr_id)
    project = binding if binding is not None else tracker.resolve_project(registry.repo_basename())
    current = next((adr for adr in tracker.list_adrs(project) if adr.id == adr_id), None)
    if current is None:
...
def sync_acceptance(tracker, issue_id: str, expected_body: str, proof: dict) -> dict:
    ...
    mutated = tracker.sync_acceptance_body(
        issue_id, expected_body, updated_body, proof, **kwargs,
    )
    return {
...
def _validate_epic_parent(parent) -> None:
    ...
    if ac_done > ac_total or (ac_total and ac_done != ac_total):
        raise SystemExit("Clôture Epic refusée : les AC propres de l'Epic sont incomplètes.")


def _validate_epic_outcome(
    outcome, *, project: Project, parent, expected: EpicClosureReceipt | None,
) -> EpicClosureOutcome:
    ...
    if expected is not None and receipt != expected:
        raise SystemExit("Clôture Epic refusée : reçu provider différent de la demande.")
    ...
    if expected is None and (
        ...
        raise SystemExit("Clôture Epic refusée : snapshot parent de reprise contradictoire.")
    if (receipt.parent_ac_done > receipt.parent_ac_total
            or (receipt.parent_ac_total
    ...
    for child in receipt.children:
        if (type(child) is not EpicClosureChild or not isinstance(child.id, str)
                or not child.id or child.state not in EPIC_CHILD_TERMINAL_STATES):
    ...
    if not isinstance(receipt.nonce, str) or not _SAFE_RECEIPT_NONCE.fullmatch(receipt.nonce):
        raise SystemExit("Clôture Epic refusée : nonce du reçu invalide.")
    closed_version = _exact_positive_version(
        outcome.closed_parent_version, "version parent clôturée",
    )
    if closed_version <= receipt.parent_version:
        raise SystemExit("Clôture Epic refusée : version clôturée non avancée.")
    if not isinstance(outcome.audit_id, str) or not _SAFE_AUDIT_ID.fullmatch(outcome.audit_id):
...
def close_epic(
    tracker, parent_id: str, *, issued_at: int | None = None, nonce: str | None = None,
) -> EpicClosureOutcome:
    ...
    if parent.state == "done":
        ...
        outcome = _validate_epic_outcome(
            prior, project=project, parent=parent, expected=None,
        )
        if outcome.closed_parent_version != parent.version:
            raise SystemExit("Clôture Epic refusée : version de reprise contradictoire.")
    ...
    if parent.state == "dropped":
        raise SystemExit("Clôture Epic refusée : un Epic dropped ne peut pas être clôturé done.")
    ...
    if not child_ids:
        raise SystemExit("Clôture Epic refusée : aucun enfant requis n'est lié à l'Epic.")
    ...
    if (not isinstance(receipt_nonce, str)
            or not _SAFE_RECEIPT_NONCE.fullmatch(receipt_nonce)):
    ...
    return _validate_epic_outcome(
        outcome, project=project, parent=parent, expected=receipt,
    )
...
def ci_gate(codehost, repo: str, sha: str, allow_no_ci: bool = False) -> dict:
    ...
    failing = [c.name for c in checks if c.status == "completed"
    ...
    return {
        ...
        "failing": failing,
        "total": len(checks),
        "reason": ("checks en cours" if pending else
                   "checks en échec" if failing else
                   "aucun check — waiver explicite (--allow-no-ci)" if waived else
                   "aucun check visible mais une check-suite existe — la CI n'a "
                   "probablement pas encore démarré ; waiver refusé, attends et "
                   "relance" if ci_seen else
                   "aucun check trouvé — rien ne prouve la CI verte (pas encore "
                   "démarrée, ou pas de CI)" if not checks else
                   "aucun check n'a conclu success (tous neutral/skipped) — rien ne "
                   "prouve la CI verte" if not proven else "tous verts"),
    }
